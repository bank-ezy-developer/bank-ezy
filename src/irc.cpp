...
// Copyright (c) 2011-2012 BEZCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BEZCoinTEST0\r");
            Send(hSocket, "WHO #BEZCoinTEST0\r");
        } else {
            // randomly join #BEZCoin00-#BEZCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #BEZCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BEZCoin%02d\r", channel_number).c_str());
        }
