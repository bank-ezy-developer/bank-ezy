...
uint256 hashGenesisBlock("0x94ab0b6985b21dc01924e8b253a58e05d915e47ab6ddb6c7d635ee5df5c14972");
...
const string strMessageMagic = "BEZCOIN Signed Message:\n";
...
{
    // BEZCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BEZCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "bez genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1534596211;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1171802;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x4fcdcd3df4bda336f57a1bbc55a9500c2504443de9a891845ab7d7bbf72d83fb"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BEZCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BEZCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BEZCoinMiner(CWallet *pwallet)
{
    printf("BEZCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BEZCoin-miner");
    ...
    try { loop {
        ...
        printf("Running BEZCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BEZCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BEZCoinMiner, pwallet));
}
